...
pub mod fam {
    ...
    pub fn light() -> FontFamily { FontFamily::Name("inter-light".into()) }
    pub fn regular() -> FontFamily { FontFamily::Proportional }
    pub fn medium() -> FontFamily { FontFamily::Name("inter-medium".into()) }
    pub fn semibold() -> FontFamily { FontFamily::Name("inter-semibold".into()) }
    pub fn bold() -> FontFamily { FontFamily::Name("inter-bold".into()) }
    pub fn mono() -> FontFamily { FontFamily::Monospace }
    pub fn space_grotesk() -> FontFamily { FontFamily::Name("space-grotesk".into()) }
    pub fn instrument() -> FontFamily { FontFamily::Name("instrument-serif".into()) }
    pub fn instrument_italic() -> FontFamily { FontFamily::Name("instrument-italic".into()) }
}
...
pub fn install(ctx: &egui::Context) {
    ...
    macro_rules! add {
        ($name:expr, $bytes:expr) => {
            fonts.font_data.insert($name.to_owned(), FontData::from_static($bytes));
        };
    }
    ...
    add!(INTER_SEMIBOLD,    include_bytes!("../assets/Inter-SemiBold.ttf"));
    add!(INTER_BOLD,        include_bytes!("../assets/Inter-Bold.ttf"));
    add!(JETBRAINS,         include_bytes!("../assets/JetBrainsMono-Regular.ttf"));
    add!(SPACE_GROTESK,     include_bytes!("../assets/SpaceGrotesk-Variable.ttf"));
    add!(INSTRUMENT_SERIF,  include_bytes!("../assets/InstrumentSerif-Regular.ttf"));
    add!(INSTRUMENT_ITALIC, include_bytes!("../assets/InstrumentSerif-Italic.ttf"));
    ...
    ctx.style_mut(|s| {
        use TextStyle::*;
        s.text_styles.insert(Heading, FontId::new(20.0, fam::bold()));
        s.text_styles.insert(Body, FontId::new(13.0, fam::regular()));
        s.text_styles.insert(Button, FontId::new(12.5, fam::semibold()));
        s.text_styles.insert(Small, FontId::new(10.5, fam::regular()));
        s.text_styles.insert(Monospace, FontId::new(11.0, fam::mono()));
    });
}
